...
enum Help {
    ...
    static let modeGroups: [FilterGroup] = [
        FilterGroup(title: "Display", entries: [
            Entry(name: "-a, --all", args: "", description: "Include inactive interfaces", tag: nil),
            Entry(name: "--json", args: "", description: "Output as JSON", tag: nil),
            Entry(name: "--no-public-ip", args: "", description: "Skip public IP lookup", tag: nil),
        ]),
        FilterGroup(title: "Detail", entries: [
            Entry(name: "<interface>", args: "", description: "Show full info for a specific interface", tag: nil),
        ]),
    ]
    ...
    static func print() {
        ...
        Swift.print()
        Swift.print("  \(styled("nic", .bold, .white))  \(styled("Show network interfaces and addresses.", .dim))")
        Swift.print()
        Swift.print("  \(styled("Usage", .bold))  \(styled("nic", .white)) \(styled("[interface]", .cyan)) \(styled("[options]", .dim))")
        Swift.print()
